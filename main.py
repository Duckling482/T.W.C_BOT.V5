...
async def update_effectif():
    ...
    while True:
        try:
            guild = channel.guild
            blocs = []
            blocs.append(await generer_bloc(guild, DIRECTION))
            blocs.append(await generer_bloc(guild, ADJOINT_DEPUTE))
            blocs.append(await generer_bloc(guild, ADJOINT_DIRECTION))
            for role_id in [EMPLOYE_SENIOR, EMPLOYE_CONFIRME, EMPLOYE, EMPLOYE_JUNIOR]:
                blocs.append(await generer_bloc(guild, role_id))
            blocs.append(await generer_bloc(guild, APPRENTI, avec_departement=False))
            blocs.append(await generer_bloc(guild, STAGIAIRE, avec_departement=False))

            personnel_role = guild.get_role(1158798630254280855)
            count = len(personnel_role.members) if personnel_role else 0
            blocs.append(f"**Total de <@&{personnel_role.id}> : {count}**\n")

            # Mémoire pour stocker les messages existants
            if not hasattr(bot, "effectif_messages"):
                bot.effectif_messages = []

            # Envoyer ou modifier les messages
            if not bot.effectif_messages:
                # Première fois : envoyer les messages et stocker leurs références
                message_buffer = ""
                for bloc in blocs:
                    if len(message_buffer) + len(bloc) < 2000:
                        message_buffer += bloc
                    else:
                        msg = await channel.send(message_buffer)
                        bot.effectif_messages.append(msg)
                        message_buffer = bloc
                if message_buffer:
                    msg = await channel.send(message_buffer)
                    bot.effectif_messages.append(msg)
            else:
                # Modifier les anciens messages
                message_buffer = ""
                i = 0
                for bloc in blocs:
                    if len(message_buffer) + len(bloc) < 2000:
                        ...
                        if i < len(bot.effectif_messages):
                            await bot.effectif_messages[i].edit(content=message_buffer)
                        else:
                            msg = await channel.send(message_buffer)
                            bot.effectif_messages.append(msg)
                        message_buffer = bloc
                ...
                # Supprimer les messages restants en trop (si moins de blocs que précédemment)
                while i < len(bot.effectif_messages):
                    await bot.effectif_messages[i].delete()
                    bot.effectif_messages.pop(i)

            await asyncio.sleep(60)
        except Exception as e:
            print(f"Erreur dans update_effectif: {e}")
            await asyncio.sleep(60)
